...
def register_learn_routes(app: Flask, path_prefix: str = "/api/learn") -> Flask:
    ...
    @app.route(
        path_prefix + "/shifu/<shifu_bid>/outlines/<outline_bid>/tokui",
        methods=["GET", "POST"],
    )
    @with_shifu_context()
    def get_or_generate_tokui_api(shifu_bid: str, outline_bid: str):
        """
        get or generate learner tokui artifact
        ---
        tags:
            - learn
        """
        ...
        if request.method == "POST":
            ...
            force_regenerate = str(request.args.get("force_regenerate", "")).lower() in {
                "true",
                "1",
            }
        return make_common_response(
    ...
    @app.route(
        path_prefix + "/shifu/<shifu_bid>/outlines/<outline_bid>/tokui/retry",
        methods=["POST"],
    )
    @with_shifu_context()
    def retry_tokui_api(shifu_bid: str, outline_bid: str):
        """
        retry learner tokui generation
        ---
        tags:
            - learn
        """
    ...
    @app.route(
        path_prefix + "/shifu/<shifu_bid>/outlines/<outline_bid>/tokui/responses",
        methods=["POST"],
    )
    @with_shifu_context()
    def save_tokui_responses_api(shifu_bid: str, outline_bid: str):
        """
        save learner tokui responses
        ---
        tags:
            - learn
        """
        ...
        payload = request.get_json() or {}
        return make_common_response(
            save_tokui_responses(app, shifu_bid, outline_bid, user_bid, payload)
        )
