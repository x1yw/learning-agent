...
def register_shifu_routes(app: Flask, path_prefix="/api/shifu"):
    ...
    @app.route(
        path_prefix + "/shifus/<shifu_bid>/outlines/<outline_bid>/tokui-template",
        methods=["GET"],
    )
    @ShifuTokenValidation(ShifuPermission.VIEW)
    @with_shifu_context()
    def get_tokui_template_api(shifu_bid: str, outline_bid: str):
        """
        get tokui template
        ---
        tags:
            - shifu
        """
        return make_common_response(get_draft_tokui_template(app, shifu_bid, outline_bid))
    ...
    @ShifuTokenValidation(ShifuPermission.EDIT)
    @with_shifu_context()
    def save_tokui_template_api(shifu_bid: str, outline_bid: str):
        """
        save tokui template
        ---
        tags:
        ...
        payload = request.get_json() or {}
        return make_common_response(
            save_draft_tokui_template(app, user_id, shifu_bid, outline_bid, payload)
        )

    @app.route(
        path_prefix
        + "/shifus/<shifu_bid>/outlines/<outline_bid>/tokui-template/preview",
        methods=["POST"],
    )
    @ShifuTokenValidation(ShifuPermission.EDIT)
    @with_shifu_context()
    def preview_tokui_template_api(shifu_bid: str, outline_bid: str):
        """
        generate tokui teacher preview
        ---
        tags:
            - shifu
        """
    ...
    @app.route(
        path_prefix
        + "/shifus/<shifu_bid>/outlines/<outline_bid>/tokui-template/image",
        methods=["POST"],
    )
    @ShifuTokenValidation(ShifuPermission.EDIT)
    @with_shifu_context()
    def generate_tokui_template_image_api(shifu_bid: str, outline_bid: str):
        """
        generate and store a tokui teacher image media ref
        ---
        tags:
            - shifu
        """
    ...
    @app.route(path_prefix + "/shifus/<shifu_bid>/tokui/validate", methods=["POST"])
    @ShifuTokenValidation(ShifuPermission.VIEW)
    def validate_shifu_tokui_api(shifu_bid: str):
        """
        validate tokui dsl
        ---
        tags:
        ...
        if not isinstance(dsl, str):
            raise_param_error("dsl")
        return make_common_response(validate_tokui_preview(app, dsl))

    @app.route(path_prefix + "/tokui/validate", methods=["POST"])
    def validate_tokui_api():
        """
        validate tokui dsl without a course-scoped permission check
        ---
        tags:
            - shifu
        """
